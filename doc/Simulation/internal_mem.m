function mem = internal_mem(Tixy, Toxy, Npar, Nnp, BW )
...
nelem = 0;
for i = 1:length(Nixy)
    nelem = max(nelem, Nif(i) * min(Tixy, Nixy(i)) * min(Tixy, Nixy(i)));
end
bw_elem = BW;
...
nelem = 0;
for i = 1:length(Nixy)
    nelem = max(nelem, Nof(i) * min(Toxy, Nixy(i)/S(i)) * min(Toxy, Nixy(i)/S(i)));
end
bw_elem = BW;
mem = mem + nelem*bw_elem;
%% KEX BUFFER
